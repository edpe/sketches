...
function draw() {
  ...
  for (var i = 0; i < 36; i++) {
    ...
    if (rectangles[i].xSize <= 1 && rectangles[i].ySize <= 1) {
      if (rectangles[i].xPos < rectangles[i].originXPos + 40) {
        rectangles[i].xPos++;
      }
      if (
        rectangles[i].xPos >= rectangles[i].originXPos + 40 &&
        rectangles[i].yPos < rectangles[i].originYPos + 40
      ) {
        rectangles[i].yPos++;
      }
    }
